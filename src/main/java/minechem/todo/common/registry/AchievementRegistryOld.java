package minechem.todo.common.registry;
...
import minechem.todo.common.achievement.ElementAchievementPage;

public class AchievementRegistryOld {
	
	private static AchievementRegistryOld instance;
	...
	private AchievementRegistryOld() {
		elementsAchievementMap = new TreeMap<Element, Achievement>();
		minechemAchievementMap = new TreeMap<String, Achievement>();
	}

	public static AchievementRegistryOld getInstance() {
		if (instance == null) {
			instance = new AchievementRegistryOld();
		}
		return instance;
	}
	...
	public Achievement addAchievement(Element element) {
		...
		Achievement achievement = (new ElementAchievement(element, position[0] - 5, position[1] - 8)).initIndependentStat();
