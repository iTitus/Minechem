...
//		RenderHelper.disableBlend();
	...
	public boolean isWrappable(ItemStack stack) {
		return stack.getItem().isEnchantable(stack) && !(stack.getItem() instanceof ItemWrapped) && getWrappedItemStack(stack) == null;
	}
	...
	public boolean setAugmentLevel(ItemStack item, IAugment augment, int level) {
		...
		if (item.getTagCompound().hasKey(augmentKey, Constants.NBT.TAG_COMPOUND)) {
			item.getTagCompound().getCompoundTag(augmentKey).setByte(ItemAugmented.level, (byte) level);
			return true;
		}
		return false;
	}
